...
export class FireService {
  ...
  uri = 'http://localhost:33764/api/Fire';
...
getAll(dateFrom: string, dateTo: string) {
  ...
  return this.http.get(`${this.uri}`+"?DateFrom="+dateFrom+"&DateTo="+dateTo, { headers: reqHeader });
}
getById(id: number) {
  let token = this.AutoRefreshToken();
  if(token == null) {this.router.navigate(['User/Login'])}
  ...
  // if(id !== 0)
  //   return this.http.get(`${this.uri}`+"?id="+id, { headers: reqHeader });
  return this.http.get(`${this.uri}`+"/GetById?id="+id, { headers: reqHeader });
}
insert(x: NewFire) {
  let token = this.AutoRefreshToken();
  if(token == null) {this.router.navigate(['User/Login'])}
  ...
  return this.http.post(`${this.uri}`, x, { headers: reqHeader });
}
insertDateEnd(id: number) {
  ...
  //var fireId = {"id": id};
  //return this.http.post(`${this.uri}`+'/InsertDateEnd?id='+id, { headers: reqHeader });
  return this.http.post(`${this.uri}`+'/InsertDateEnd?id='+id, null, { headers: reqHeader });
}
delete(id: string) {
  let token = this.AutoRefreshToken();
  if(token == null) {this.router.navigate(['User/Login'])}
  ...
  return this.http.delete(`${this.uri}`+'?id='+id, { headers: reqHeader });
}
